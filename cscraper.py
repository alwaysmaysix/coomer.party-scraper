...
from scraping_utils.scraping_utils import (
    compute_file_hashes, DownloadThread, multithread_download_urls_special, IMG_EXTS, VID_EXTS, TOO_MANY_REQUESTS, NOT_FOUND, THROTTLE_TIME
)
...
def main(url, dst, vids):
    ...
    if len(url_sections) < 5 or url_sections[-2] != 'user':
        stdout.write('[main] INFO: The URL must be for a creator, not a specific post or media.\n')
        return
